function [H, K] = getInputConstraints(object,varargin)
% getInputConstraints   Gets the H and K matrices defining input constraints
%
% [H, K] = getInputConstraints(OBJ)
% Gets all constraints involving system inputs. H and K are cell arrays, 
...
% H u  <= K
%
% being u the system inputs. OBJ is the constraints object.
%
% [H, K] = getInputConstraints(OBJ,T)
% Gets all constraints involving system inputs at time instant T. 
% T must be a scalar between 0 and the time horizon N. 
% H and K are matrices defining all input constraints in the form:
%
% H u  <= K
%
% [H, K] = getInputConstraints(OBJ,'soft')
% [H, K] = getInputConstraints(OBJ,'hard')
% Gets only soft or hard input constraints.
%
% [H, K] = getInputConstraints(OBJ,'soft',T)
% [H, K] = getInputConstraints(OBJ,'hard',T)
% Gets only soft or hard input constraints at time instant T.
%
...
if nargin == 1
    ...
    for i = 1:object.N+1
        ...
        idx =  ismember(object.H{i}(:,[1:nx nx+nu+1:end]),zeros(1,nx+np+nd+ny+nref),'rows');
        
        H{i} = object.H{i}(idx,nx+1:nx+nu);
        K{i} = object.K{i}(idx);
...
elseif nargin == 2
    ...
    if isnumeric(varargin{1})
        ...
        idx =  ismember(object.H{T+1}(:,[1:nx nx+nu+1:end]),zeros(1,nx+np+nd+ny+nref),'rows');
        
        H = object.H{T+1}(idx,nx+1:nx+nu);
        K = object.K{T+1}(idx);
