function [H, K] = getStateConstraints(object,varargin)
% getStateConstraints   Gets the H and K matrices defining state constraints
%
% [H, K] = getStateConstraints(OBJ)
% Gets all constraints involving system states. H and K are cell arrays, 
...
% H x  <= K
%
% being x the system states. OBJ is the constraints object.
%
% [H, K] = getStateConstraints(OBJ,T)
% Gets all constraints involving system states at time instant T. 
% T must be a scalar between 0 and the time horizon N. 
% H and K are matrices defining all state constraints in the form:
%
% H x  <= K
%
% [H, K] = getStateConstraints(OBJ,'soft')
% [H, K] = getStateConstraints(OBJ,'hard')
% Gets only soft or hard state constraints.
%
% [H, K] = getStateConstraints(OBJ,'soft',T)
% [H, K] = getStateConstraints(OBJ,'hard',T)
% Gets only soft or hard state constraints at time instant T.
%
% See also: constraints/getInputConstraints,
...
if nargin == 1
    ...
    for i = 1:object.N+1
        ...
        idx =  ismember(object.H{i}(:,nx+1:end),zeros(1,nu+np+nd+ny+nref),'rows');
        
        H{i} = object.H{i}(idx,1:nx);
        K{i} = object.K{i}(idx);
...
elseif nargin == 2
    ...
    if isnumeric(varargin{1})
        ...
        idx =  ismember(object.H{T+1}(:,nx+1:end),zeros(1,nu+np+nd+ny+nref),'rows');
        
        H = object.H{T+1}(idx,1:nx);
        K = object.K{T+1}(idx);
