...
deltaRealIn = max(V)-min(V);

meanRealIn = [(max(V)+min(V))/2]';
...
fin = fopen([getvhdlpath(),'discontinuousPwasFunction/findDynamic.vhd'],'r');
...
while 1
    ...
    if strcmp(tline,'--- BEGIN MATLABGEN ---')
        ...
        if count == 0
            ...
            fprintf(fout,'type HKType_row_PWAS is array (0 to N_DIM_PWAS) of signed(N_BIT_COEFF_PWAS-1 downto 0);\n');
            if size(Mint,1) == 1 % TO DO mettere a posto il fatto che ci possa essere solo un edge
                fprintf(fout,'type HKType_PWAS is array (0 to N_EDGE) of HKType_row_PWAS;\n');
            else
                fprintf(fout,'type HKType_PWAS is array (0 to N_EDGE-1) of HKType_row_PWAS;\n');
            end
            fprintf(fout,'type HK_shift_Type_PWAS is array (0 to N_DIM_PWAS) of integer;\n');
            fprintf(fout,'constant HK : HKType_PWAS :=(('); 
            ...
            fprintf(fout,'constant shift_findDyn : HK_shift_Type_PWAS :=(');
