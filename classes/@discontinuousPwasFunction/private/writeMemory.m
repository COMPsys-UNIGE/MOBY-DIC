...
for k=1:object.nDyn
    ...
    for id = 1:numel(idx)
        ...
        if strcmpi(outputRepresentation,'signed')
            wbin{k,id} = decimal2signed(wcur,nbit_coeff,fpp);
        else
            wbin{k,id} = decimal2unsigned(wcur,nbit_coeff,fpp);
        end
    end
...
if strcmpi(opts.architecture,'fast')
    ...
    fin = fopen([getvhdlpath(),'discontinuousPwasFunction/Memory.vhd'],'r');
    ...
    while 1
        ...
        if strcmp(tline,'--- BEGIN MATLABGEN ---')
            ...
            fprintf(fout,'\ttype big_ROM_type is array (0 to N_DYN_PWAS-1) of ROM_type;\n\n');
            
            for id = 1:nfun
                fprintf(fout,'\tconstant ROM_%d : big_ROM_type:=((\n\t',id-1);
                for k=1:object.nDyn-1
                    for i = 1:numel(wbin{id})-1
                        fprintf(fout,'"%s", ',wbin{k,id}(i).bin);
                        if mod(i,10) == 0
                            fprintf(fout,'\n\t');
                        end
                    end
                ...
                for i = 1:numel(wbin{id})-1
                    fprintf(fout,'"%s", ',wbin{end,id}(i).bin);
                    if mod(i,10) == 0
                        fprintf(fout,'\n\t');
                    end
                end
                fprintf(fout,'"%s"));',wbin{end,id}(end).bin);
                fprintf(fout,'\n');
            end
            ...
            if strcmpi(outputRepresentation,'signed')
                for i = 1:nfun
                    fprintf(fout,'\t\t\t\tres(%d)(j) <= ROM_%d(dynamic)(to_integer(addr(j)));\n',i-1,i-1);
                end
            else
                for i = 1:nfun
                    fprintf(fout,'\t\t\t\tres(%d)(j) <= ''0''&ROM_%d(dynamic)(to_integer(addr(j)));\n',i-1,i-1);
                end
            end
            fprintf(fout,'\t\t\tend loop;\n');
            fprintf(fout,'\t\tend if;\n');
    ...
    while 1
        ...
        if strcmp(tline,'--- BEGIN MATLABGEN ---')
            ...
            if strcmpi(outputRepresentation,'signed')
                for i = 1:nfun
                    fprintf(fout,'\t\t\t\tres(%d)(state) <= ROM_%d(dynamic)(to_integer(addr(state)));\n',i-1,i-1);
                end
            else
                for i = 1:nfun
                    fprintf(fout,'\t\t\t\tres(%d)(state) <= ''0''&ROM_%d(dynamic)(to_integer(addr(state)));\n',i-1,i-1);
                end
            end
            fprintf(fout,'\t\t\tend if;\n');
            fprintf(fout,'\t\tend if;\n');
