...
for i = 1:N
    ...
    try
        l0R = length(ineqR);
        [~, ineqR_tmp] = object.constr.getStateConstraints(i); 
        ineqR = [ineqR; ineqR_tmp];
        l1R = length(ineqR);

        if object.options.tracking
            try
                ineqL(l0R+1:l1R, Nu*nu + (i-1)*nx + 1 : Nu*nu + i*nx - nu) = H{i+1}(any((H{i+1}(:, 1:nx-nu)), 2), 1:nx-nu);
    %             ineqL(length(cell2mat(K(1:i-1)))+1:length(cell2mat(K(1:i))), Nu*nu + i*nx -nu + 1 : Nu*nu + i*nx) = H{i}(:, nx-nu+1:nx);
            ...
            try
                if any(H{i+1}(:, 1:nx))
                    ineqL(l0R+1:l1R, Nu*nu + (i-1)*nx + 1 : Nu*nu + i*nx) = H{i+1}(any((H{i+1}(:, 1:nx)), 2), 1:nx);
    %             else
...
ineqL = [ineqL, zeros(size(ineqL,1), Nu*nu + N*nx - size(ineqL,2))];
...
if nSoft ~= 0 && algo ~= "ADMM"
    ...
    Qsoft = zeros(N*nSoft);
    for i=1:nSoft:N*nSoft
        Qsoft(i:i+nSoft-1,i:i+nSoft-1) = rho;
    end
    QP1 = [QP1, zeros(size(QP1,1),N*nSoft); zeros(N*nSoft,size(QP1,2)), Qsoft];

    QP2 = [QP2; zeros(N*nSoft,1)];

    eqL = [eqL, zeros(size(eqL,1),N*nSoft)];

    ineqLlength = size(ineqL, 1);
    ineqL = [ineqL, zeros(size(ineqL,1), N*nSoft)];
    for i = 1:N
