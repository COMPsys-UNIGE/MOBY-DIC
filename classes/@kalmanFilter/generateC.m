...
if circuit_parameters.generate_main
    ...
    fprintf(f,'update(inVector,state_predict,state_update);\n');
...
fprintf(f,'void update(float observerInput[nIn_OBS], float current_state[nOut_OBS], float update_state[nOut_OBS]);\n');
fprintf(f,'#endif\n');
fclose(f);
...
fprintf(f,'const float Fupdate[nOut_OBS][nOut_OBS+nIn_OBS] = {{');
for i=1:nOut-1
    for j=1:nIn+nOut-1
        fprintf(f,'%.10e,',F(i,j));
    end
...
fprintf(f,'const float Gupdate[nOut_OBS] = {');
for i=1:nOut-1
    fprintf(f,'%.10e,\n',Gyobs(i));
end
fprintf(f,'%.10e};\n\n',Gyobs(end));
...
fprintf(f,'\tfor(j=0;j<nOut_OBS;j++)\n');
fprintf(f,'\t\tupdate_state[i] += Fupdate[dynamic][i][j]*current_state[j];\n');
fprintf(f,'\tfor(j=0;j<nIn_OBS;j++)\n');
fprintf(f,'\t\tupdate_state[i] += Fupdate[dynamic][i][j+nOut_OBS]*observerInput[j];\n');
fprintf(f,'\tupdate_state[i] += Gupdate[dynamic][i];\n');
fprintf(f,'}\n}\n');
...
memory_size = numel([Aobs Bobs Gxobs; Cobs Dobs Gyobs]);
