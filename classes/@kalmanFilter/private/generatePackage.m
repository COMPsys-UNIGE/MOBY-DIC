...
ndec_coeffUpdate = nbit_coeff-nint_coeffUpdate;

shiftMulPred = [ndec 0]+ndec_coeffPred; % decimal after multiplication
preshiftPred = max(shiftMulPred)-shiftMulPred;

shiftMulUpdate = [ndec 0]+ndec_coeffUpdate;
preshiftUpdate = max(shiftMulUpdate)-shiftMulUpdate;
...
MPred = [FintPred GintPred];
MUpdate = [FintUpdate GintUpdate];
...
nBusMacUpdate = nbit+nbit_coeff+ceil(log2(nx+np+nd+nu+ny))+max(preshiftUpdate)-min(preshiftUpdate);
...
fprintf(fout,'constant nBusMacUpdate_OBS : integer := %d;\n',nBusMacUpdate);% N_BIT_OBS+N_BIT_COEFF_OBS+nx_OBS+np_OBS+nd_OBS+nu_OBS+ny_OBS+maxShiftUpdate_OBS-minShiftUpdate_OBS;\n\n');
...
betaPredBin = decimal2signed(betaPred,nBusMacPredict,max(shiftMulPred)+alphaPredBin.pointposition);
fprintf(fout,'constant alphaPred_OBS : std_logic_vector(N_BIT_COEFF_OBS-1 downto 0) := "%s";\n\n',alphaPredBin.bin);
fprintf(fout,'constant betaPred_OBS : std_logic_vector(nBusMacPredict_OBS-1 downto 0) := "%s";\n\n',betaPredBin.bin);

% Write Mpred matrix
fprintf(fout,'constant predictMatrix : observerMatrixType_OBS := (( ');
for i=1:size(MPred,1)
    for j=1:size(MPred,2)
        num = decimal2signed(MPred(i,j),nbit_coeff,0);%ndec_coeffPred(j));
...
alphaUpdateBin = decimal2signed(alphaUpdate,nbit_coeff);
betaUpdateBin = decimal2signed(betaUpdate,nBusMacUpdate,max(shiftMulUpdate)+alphaUpdateBin.pointposition);
fprintf(fout,'constant alphaUpdate_OBS : std_logic_vector(N_BIT_COEFF_OBS-1 downto 0) := "%s";\n\n',alphaUpdateBin.bin);
fprintf(fout,'constant betaUpdate_OBS : std_logic_vector(nBusMacUpdate_OBS-1 downto 0) := "%s";\n\n',betaUpdateBin.bin);

% Write Mupdate matrix
fprintf(fout,'constant updateMatrix : observerMatrixType_OBS := (( ');
for i=1:size(MUpdate,1)
    for j=1:size(MUpdate,2)
        num = decimal2signed(MUpdate(i,j),nbit_coeff,0);%ndec_coeffUpdate(j));
        fprintf(fout,'"%s"',num.bin);
        if j ~= size(MUpdate,2)
            fprintf(fout,',');
        else
...
fprintf(fout,'\tconstant nDecOutUpdate_OBS : integer := %d;\n',alphaUpdateBin.pointposition+max(shiftMulUpdate));

% fprintf(fout,'\tconstant nDecMacOutPred : integer := %d;\n',max(shiftMulPred));
% fprintf(fout,'\tconstant nDecMacOutUpdate : integer := %d;\n',max(shiftMulUpdate));
% fprintf(fout,'\tconstant nDec : int_array := (');
...
for i = 1:nx+nd+np+nu+ny
    fprintf(fout,'%d,',preshiftPred(i));
end
fprintf(fout,'%d);\n',preshiftPred(nx+nd+np+nu+ny+1));

fprintf(fout,'\tconstant preShiftUpdate_OBS : int_shift_array_OBS := (');
for i = 1:nx+nd+np+nu+ny
    fprintf(fout,'%d,',preshiftUpdate(i));
end
fprintf(fout,'%d);\n',preshiftUpdate(nx+nd+np+nu+ny+1));
fprintf(fout,'\n');
