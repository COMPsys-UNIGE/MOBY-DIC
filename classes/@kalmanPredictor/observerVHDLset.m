function optout = observerVHDLset(object,opts)
% observerVHDLset   Checks and sets the options for the generation of VHDL code for
%                   the obeserver object
%
% OPTOUT = observerVHDLset(object,opts)
% FUN is an obeserver object, OPTS is the structure defining the options.
% See the documentation of obeserver.generateVHDL for a list of all
% structure fields.
...
minVal = [optout.range.xmin(:);optout.range.umin(:);optout.range.pmin(:);...
    optout.range.dmin(:);optout.range.ymin(:)];
maxVal = [optout.range.xmax(:);optout.range.umax(:);optout.range.pmax(:);...
    optout.range.dmax(:);optout.range.ymax(:)];

signVect = minVal >= 0;

noptMin = ceil(log2((2*signVect-1).*minVal + signVect)+1);

signVect = maxVal >= 0;

noptMax = ceil(log2((2*signVect-1).*maxVal + signVect)+1);

nint = max([noptMin;noptMax]');
...
if numel(optout.inputRange.min) == 1
    optout.inputRange.min = repmat(optout.inputRange.min,nu+ny+np,1);
    optout.inputRange.max = repmat(optout.inputRange.max,nu+ny+np,1);
end
...
if ~isfield(opts,'coeffResolution') || isempty(opts.coeffResolution)
    ...
    if opts.coeffResolution < optout.inputResolution
        error('OPTS.coeffResolution must be greater than or equal to OPTS.inputResolution');
    end
...
if numel(optout.outputRange.min) == 1
    optout.outputRange.min = repmat(optout.outputRange.min,nx+nd,1);
    optout.outputRange.max = repmat(optout.outputRange.max,nx+nd,1);
end
