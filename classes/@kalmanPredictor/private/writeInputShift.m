...
fprintf(fout,'Port(');
fprintf(fout,'u_in : in compositeInputType_OBS;\n');
fprintf(fout,'	u_out : out compositeInputType_OBS');
fprintf(fout,');\n');
fprintf(fout,'end inputShift;\n\n');

fprintf(fout,'architecture Behavioral of inputShift is\n');
if np+nu+ny ~= 1
fprintf(fout,'type shiftConstType is array(0 to np_OBS+nu_OBS+ny_OBS-1) of std_logic_vector(N_BIT_OBS downto 0);\n');
...
fprintf(fout,'"%s");\n',num.bin);
    ...
    fprintf(fout,'constant shiftConst : std_logic_vector(N_BIT_OBS downto 0) := "%s";\n',num.bin);
end
fprintf(fout,'begin\n');
...
fprintf(fout,'\nbegin\n');
    ...
    fprintf(fout,'for i in 0 to nu_OBS+np_OBS+ny_OBS-1 loop\n');
    if signed_
    fprintf(fout,'\tu_out(i) <= std_logic_vector(resize(resize(signed(u_in(i)),N_BIT_OBS+1)+signed(shiftConst');
    if np+nu+ny ~= 1
    fprintf(fout,'(i)');
    end
