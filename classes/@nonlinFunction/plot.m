...
%
% plot(OBJ)
% Plots all the components of the vector non linear function OBJ.
%
% plot(OBJ,IDX)
% Plots the components of the vector non linear function specified in the
% vector of indices IDX. IDX must be a vector of indices in the range 1, NY.
%
% plot(OBJ,SEC)
% Plots all the components of the vector non linear function OBJ in a 
...
% If the domain is already hyper-rectangular, retrieve the bounds...
...
inside = zeros(1,size(p,2));

% If domain is not hyper-rectangular, remove points outside P
if exist('P','var')
    
    for i = 1:size(p,1)
        inside(i) = all(object.domain.Hd*p(:,i) <= object.domain.Kd);
    end
