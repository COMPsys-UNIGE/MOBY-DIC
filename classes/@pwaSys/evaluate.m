...
%
%   DXDT = A^{(i)} X + B^{(i)} U + E_x^{(i)} P + F_x^{(i)} D + G_x^{(i)}
%   if H^{(i)} X <= K^{(i)}
%
...
% is computed as:
%
%   XNEXT = A^{(i)} X + B^{(i)} U + E_x^{(i)} P + F_x^{(i)} D + G_x^{(i)}
%
% where X, U, P and D are the system state, input, parameters and
% unmeasurable inputs at current time instant, respectively.
% OBJ is the ltiSys object.
...
npts = numel(x)/object.nx;

xnext = zeros(object.nx,npts);
dyn = object.findDynamics(x,p,d);

for i=1:npts
    xnext(:,i) = object.A{dyn(i)}*x(:)+object.B{dyn(i)}*u(:)+object.Ex{dyn(i)}*p(:)+object.Fx{dyn(i)}*d(:)+object.Gx{dyn(i)};
end