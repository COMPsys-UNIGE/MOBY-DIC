...
% SYNTAX
%
% [Hint Kint Fint Gint alpha beta nint nintcoeff ndecout] = ...
...
function [Hint Kint Fint Gint alpha beta nintcoeff] = ...
    quantizeData(object,H,K,F,G,D,nbit,nbitcoeff)
...
nintcoeffKG(nintcoeffKG == -Inf) = nbitcoeff;
...
nintcoeffHF(nintcoeffHF == -Inf) = nbitcoeff;
% Number of bits remaining for the decimal part
...
% x_cir = A x
% we chose scalexy power of two => A is power of two => chenge intbitcoeff
% A = scaleKG./scaleHF;
% Alog = log2(A);
% if A ~= 2.^Alog
%     error('Error in this function!');
% end
...
% add A matrix scale contribute
% nintcoeff(1:ndim) = nintcoeff(1:ndim)+Alog;
% 
% if any(nintcoeff < 0)
%     toSum = min(nintcoeff);
%     nintcoeff = nintcoeff-toSum;
%     alpha = alpha*2^toSum;
% end
