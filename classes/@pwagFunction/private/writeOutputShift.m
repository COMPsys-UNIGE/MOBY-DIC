...
fprintf(fout,'Port(');
    ...
    fprintf(fout,'y_in : in y_matrix_PWAG;\n');
    fprintf(fout,'	y_out : out y_matrix_PWAG');
fprintf(fout,');\n');
fprintf(fout,'end outputShift;\n\n');

fprintf(fout,'architecture Behavioral of outputShift is\n');
if nout ~= 1
fprintf(fout,'type shiftConstType is array(0 to N_FUN_PWAG-1) of signed(N_BIT_OUT_PWAG downto 0);\n');
...
fprintf(fout,'"%s");\n',num.bin);
    ...
    fprintf(fout,'constant shiftConst : signed(N_BIT_OUT_PWAG downto 0) := "%s";\n',numi.bin);
end
fprintf(fout,'begin\n');
fprintf(fout,'main_process : process(y_in)\n');
fprintf(fout,'variable tmp : std_logic_vector(N_BIT_OUT_PWAG downto 0);\nbegin\n');

fprintf(fout,'for i in 0 to N_FUN_PWAG-1 loop\n');
    fprintf(fout,'\ttmp := std_logic_vector(resize(signed(y_in(i)),N_BIT_OUT_PWAG+1)+shiftConst');
    if nout ~= 1
        fprintf(fout,'(i)');
    end
    fprintf(fout,');\n');
    fprintf(fout,'\ty_out(i) <= signed(tmp(N_BIT_OUT_PWAG-1 downto 0));\n');
