...
circuit_parameters = object.switchedApproxMPCctrlVHDLset(circuit_parameters);
...
%generate main block
if strcmp(circuit_parameters.inputRepresentation,'signed')
    copyfile([getvhdlpath,'/switchedApproxMPCctrl/switchedControllerCircuit_signed.vhd'],[folder,'/controllerCircuit.vhd']);
else
    copyfile([getvhdlpath,'/switchedApproxMPCctrl/switchedControllerCircuit_unsigned.vhd'],[folder,'/controllerCircuit.vhd']);
end
...
if generateInterface
    ...
    object.writeFindDynamic(circuit_parameters);
    copyfile([getvhdlpath,'/switchedApproxMPCctrl/mulBank.vhd'],[folder,'/mulBank.vhd']);
...
%     object.writeOutputShift(meanOutADC,circuit_parameters);
    ...
    fprintf(fout,'\t - Multiplier(s) = %d (%d x %d bits)\n\n',funOut.multipliers,max(nbit,nbit_coeff),max(nbit,nbit_coeff));
