...
fin = fopen([getvhdlpath(),'switchedApproxMPCctrl/findDynamic.vhd'],'r');
...
while 1
    ...
    if strcmp(tline,'--- BEGIN MATLABGEN ---')
        ...
        if count == 0
            ...
            fprintf(fout,'type HKType_row_CTRL is array (0 to N_DIM_CTRL) of signed(N_BIT_COEFF_CTRL-1 downto 0);\n');
            if size(Mint,1) == 1 % TO DO mettere a posto il fatto che ci possa essere solo un edge
                fprintf(fout,'type HKType_CTRL is array (0 to N_EDGE) of HKType_row_CTRL;\n');
            else
                fprintf(fout,'type HKType_CTRL is array (0 to N_EDGE-1) of HKType_row_CTRL;\n');
            end
            fprintf(fout,'type HK_shift_Type_CTRL is array (0 to N_DIM_CTRL) of integer;\n');
            fprintf(fout,'constant HK : HKType_CTRL :=(('); 
            ...
            fprintf(fout,'constant shift_findDyn : HK_shift_Type_CTRL :=(');
