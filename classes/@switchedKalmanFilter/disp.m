...
% disp   Displays some information about the kalmanFilter object
%
% disp(OBJ)
...
if object.nu == 0
    disp('Empty switched kalmanFilter object');
else
    disp('switched Kalman filter for a system with: ')
    disp([' - ',num2str(object.nx),' state variables']);
    disp([' - ',num2str(object.nu),' input variables']);
    disp([' - ',num2str(object.nu),' output variables']);
