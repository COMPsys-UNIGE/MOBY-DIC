...
if circuit_parameters.generate_main
 ...
 fprintf(f,'update(inVector,state_predict,state_update,i);\n');
    fprintf(f,'scaleOutput(outADC,state_update);\n');
    fprintf(f,'setOutputDAC(outADC);\n');
    fprintf(f,'i = findDynamic(inVector,state_update);\n');
...
fprintf(f,'void update(float observerInput[nIn_OBS], float current_state[nOut_OBS], float update_state[nOut_OBS],int dynamic);\n');
fprintf(f,'#endif\n');
fclose(f);
...
fprintf(f,'const float Fupdate[nDyn_OBS][nOut_OBS][nOut_OBS+nIn_OBS] = {{{');
for k=1:object.nDyn
    CCobs = Cobs{k}(object.ny+1:end,:);
DDobs = Dobs{k}(object.ny+1:end,:);
    F = [CCobs DDobs];
...
fprintf(f,'const float Gupdate[nDyn_OBS][nOut_OBS] = {{');
for k=1:object.nDyn
    Gxobs = Gyobs{k}(object.ny+1:end,:);
    for i=1:nOut-1
        fprintf(f,'%.10e,\n',Gxobs(i));
    end
    fprintf(f,'%.10e}',Gxobs(end));
...
fprintf(f,'int i,j;\n');
fprintf(f,'for(i=0;i<nOut_OBS;i++)\n{\n');
fprintf(f,'\tupdate_state[i] = 0;\n');
fprintf(f,'\tfor(j=0;j<nOut_OBS;j++)\n');
fprintf(f,'\t\tupdate_state[i] += Fupdate[i][j]*current_state[j];\n');
fprintf(f,'\tfor(j=0;j<nIn_OBS;j++)\n');
fprintf(f,'\t\tupdate_state[i] += Fupdate[i][j+nOut_OBS]*observerInput[j];\n');
fprintf(f,'\tupdate_state[i] += Gupdate[i];\n');
fprintf(f,'}\n}\n');
...
memory_size = numel([Aobs{1} Bobs{1} GxobsAll{1}; Cobs{1} Dobs{1} Gyobs{1}])*object.nDyn;
