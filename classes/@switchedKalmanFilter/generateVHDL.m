...
FintPred = cell(ndyn,1);
GintPred = cell(ndyn,1);
alphaPred = cell(ndyn,1);
betaPred = cell(ndyn,1);
nint_coeffPred = cell(ndyn,1);

for i=1:ndyn
    
    Ascale = inv(alphaZ)*Aobs{i}*alphaZ;
    Bscale = inv(alphaZ)*Bobs{i}*alphaQ;%.*repmat(mulScale,nx+nd,1)./repmat(outScale(:),1,np+ny+nu);
    % Gxscale = Gxobs./outScale'+meanRealOut./outScale'-Bobs*meanRealIn./outScale'-Aobs*meanRealOut(:)./outScale';
    Gxscale = -inv(alphaZ)*meanRealOut+inv(alphaZ)*Aobs{i}*meanRealOut+inv(alphaZ)*Gxobs{i}+inv(alphaZ)*Bobs{i}*meanRealIn;
    
    % compute rescale matrix and compute bit
    [FintPred{i}, GintPred{i}, alphaPred{i}, betaPred{i}, nint, nint_coeffPred{i}] = quantizeData([Ascale Bscale],Gxscale,bound,nbit,nbit_coeff);
    
end


FintUpdate = cell(ndyn,1);
GintUpdate = cell(ndyn,1);
alphaUpdate = cell(ndyn,1);
betaUpdate = cell(ndyn,1);
nint_coeffUpdate = cell(ndyn,1);

for i=1:ndyn
    
    % Take only last rows of matrices
    Cobs_ = Cobs{i}(object.ny+1:end,:);
    Dobs_ = Dobs{i}(object.ny+1:end,:);
    Gyobs_ = Gyobs{i}(object.ny+1:end,:);
    
    Cscale = inv(alphaZ)*Cobs_*alphaZ;
    Dscale = inv(alphaZ)*Dobs_*alphaQ;
    Gyscale = -inv(alphaZ)*meanRealOut+inv(alphaZ)*Cobs_*meanRealOut+inv(alphaZ)*Gyobs_+inv(alphaZ)*Dobs_*meanRealIn;
    
    % compute rescale matrix and compute bit
    [FintUpdate{i}, GintUpdate{i}, alphaUpdate{i}, betaUpdate{i}, nint, nint_coeffUpdate{i}] = quantizeData([Cscale Dscale],Gyscale,bound,nbit,nbit_coeff);
...
if all(size(initialState) == [1,nx+nd])
...
% %generate main block
% object.writePredictor(folder);

% copy other file
copyfile([getvhdlpath,'/switchedKalmanFilter/shifterMACPredictor.vhdl'],[folder,'/shifterMACPredictor.vhdl']);
copyfile([getvhdlpath,'/switchedKalmanFilter/shifterMACUpdate.vhdl'],[folder,'/shifterMACUpdate.vhdl']);
copyfile([getvhdlpath,'/switchedKalmanFilter/statePredict.vhdl'],[folder,'/statePredict.vhdl']);
copyfile([getvhdlpath,'/switchedKalmanFilter/stateUpdate.vhdl'],[folder,'/stateUpdate.vhdl']);
copyfile([getvhdlpath,'/switchedKalmanFilter/switchedKalmanFilter.vhdl'],[folder,'/switchedKalmanFilter.vhdl']);
...
if generateInterface
    ...
    memory_size = 2*(nx+nd)*(nx+nd+np+nu+ny+1)+(nedge*(nx+nd+np+1));
    
    latency = (nx+nd+np+nu+ny+2)+(nedge+3);
    Multiplier = (nx+nd+np);
    ...
    copyfile([getvhdlpath,'/switchedKalmanFilter/mulBank.vhdl'],[folder,'/mulBank.vhdl']);
