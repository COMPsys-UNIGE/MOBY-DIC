classdef switchedKalmanFilter < observer
    % switchedKalmanFilter  switched Kalman filter for the estimation of the states
    %                of a pwa system
    %
    % This object represents a switched Kalman filter for the estimation of
    ...
    % The switched Kalman filter is a dynamical system in the form:
    %
    ...
    % the Kalman filter.
    %
    % OBJ = switchedKalmanFilter()
    % Builds an empty switchedKalmanFilter object OBJ.
    %
    % OBJ = switchedKalmanFilter(SYS,Ts,K)
    % Builds a switchedKalmanFilter object OBJ by specifying the pwaSys object
    ...
    %
    % OBJ = switchedKalmanFilter(SYS,Ts,Q,R)
    % Builds a switchedKalmanFilter object OBJ by specifying the pwaSys object
    % (SYS) whose states need to be estimated, the observer sampling time
    ...
    % 'kalman'.
    %
    % switchedKalmanFilter methods:
    %   computeOutput - Computes the switched Kalman filter output
    %   disp - displays information about switchedKalmanFilter object
    ...
    %
    % The switchedKalmanFilter object is derived from observer and inherits
    % all its methods.
    %
    % See also switchedKalmanPredictor.
    ...
    methods
        ...
        function obs = switchedKalmanFilter(varargin)
            ...
            elseif nargin == 3
                ...
                for i=1:nDyn
                    ...
                    obs.filters(i).H = H{i};
                    obs.filters(i).K = K{i};
                    obs.filters(i).kFilter = kalmanFilter(linsys,obs.Ts,kk{i});
            ...
            elseif nargin == 4
                ...
                for i=1:nDyn
                    ...
                    obs.filters(i).H = H{i};
                    obs.filters(i).K = K{i};
                    obs.filters(i).kFilter = kalmanFilter(linsys,obs.Ts,Q{i},R{i});
