...
% generate package
object.generatePackage(folder,FintPred, GintPred, alphaPred, betaPred, nbit, nint, nbit_coeff, nint_coeffPred,initialState,sampling_latency);

% %generate main block
% object.writePredictor(folder);

% copy other file
copyfile([getvhdlpath,'/switchedKalmanPredictor/shifterMACPredictor.vhdl'],[folder,'/shifterMACPredictor.vhdl']);
copyfile([getvhdlpath,'/switchedKalmanPredictor/statePredict.vhdl'],[folder,'/statePredict.vhdl']);
copyfile([getvhdlpath,'/switchedKalmanPredictor/switchedKalmanPredictor.vhdl'],[folder,'/switchedKalmanPredictor.vhdl']);

memory_size = (nx+nd)*(nx+nd+np+nu+ny+1);
...
if generateInterface
    ...
    memory_size = (nx+nd)*(nx+nd+np+nu+ny+1)+(nedge*(nx+nd+np+1));
    ...
    object.writePredictorInterface(folder);
    copyfile([getvhdlpath,'/switchedKalmanPredictor/mulBank.vhdl'],[folder,'/mulBank.vhdl']);
