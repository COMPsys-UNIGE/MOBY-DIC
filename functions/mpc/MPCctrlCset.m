...
% MPCset   Checks and sets the options for the generation of C code for
%          the MPCctrl object
%
% OPTOUT = MPCctrlCset(FUN,OPTS)
% FUN is a MPCctrl object, OPTS is the structure defining the options.
% See the documentation of MPCctrl.generateC for a list of all
% structure fields.
...
if isa(fun, 'implicitMPCctrl')    
    if ~isfield(opts,'range') || isempty(opts.range)
        error('OPTS.range must be a structure with fields ''min'' and ''max''');
    else
        if ~isstruct(opts.ADMMparameters)
            error('OPTS.range must be a structure with fields ''min'' and ''max''');
        else
            optout.range = opts.range;
        end
    end
