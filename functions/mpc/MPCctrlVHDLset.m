function optout = MPCctrlVHDLset(fun,opts)
...
% FUN is a controller object, OPTS is the structure defining the options.
% See the documentation of explicitMPCctrl.generateVHDL and 
% implicitMPCctrl.generateVHDL for a list of all structure fields.
...
if isa(fun, 'explicitMPCctrl')
    ...
    nxref = numel(fun.getTrackingVariable);
...
if ~isfield(opts,'defaultOutput') || isempty(opts.defaultOutput)
    ...
    if isa(fun, 'explicitMPCctrl') && (numel(opts.defaultOutput) ~= nfun)
