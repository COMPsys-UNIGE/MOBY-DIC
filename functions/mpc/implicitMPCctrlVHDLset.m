function optout = implicitMPCctrlVHDLset(fun,opts)
% MPCset   Checks and sets the options for the generation of VHDL code for
%          the MPCctrlVHDLset object
%
% OPTOUT = MPCctrlVHDLset(FUN,OPTS)
% FUN is a implicitMPCctrl object, OPTS is the structure defining the options.
% See the documentation of MPCctrlVHDLset.generateVHDL for a list of all
% structure fields.
...
if ~isfield(opts,'inputIntResolution') || isempty(opts.inputIntResolution)
    optout.inputIntResolution = 5;
else
    if ~isnumeric(opts.inputIntResolution)
        error('OPTS.inputIntResolution must be an integer');
    end
    if opts.inputIntResolution ~= floor(opts.inputIntResolution)
        error('OPTS.inputIntResolution must be an integer');
    end
    if opts.inputIntResolution < 1
        error('OPTS.inputIntResolution must be greater than zero');
    end
    if opts.inputIntResolution > opts.inputIntResolution
        error('OPTS.inputIntResolution must be smaller than OPTS.inputResolution');
    end
    optout.inputIntResolution = opts.inputIntResolution;
end
...
% ndim = fun.getNumberOfDimensions();
% nfun = fun.getNumberOfInputs;
...
%     optout.defaultOutput = zeros(1,nfun);
% else
%     if numel(opts.defaultOutput) ~= nfun
%         error(['defaultOutput must be a vector with ',num2str(nfun),' elements']);
%     else
...
if ~isfield(opts,'outputIntResolution') || isempty(opts.outputIntResolution)
    optout.outputIntResolution = 5;
else
    if ~isnumeric(opts.outputIntResolution)
        error('OPTS.outputIntResolution must be an integer');
    end
    if opts.outputIntResolution ~= floor(opts.outputIntResolution)
        error('OPTS.outputIntResolution must be an integer');
    end
    if opts.outputIntResolution < 1
        error('OPTS.outputIntResolution must be greater than zero');
    end
    if opts.outputIntResolution > opts.outputIntResolution
        error('OPTS.outputIntResolution must be smaller than OPTS.OutputResolution');
    end
    optout.outputIntResolution = opts.outputIntResolution;
end
...
if optout.inputResolution > optout.coeffResolution
