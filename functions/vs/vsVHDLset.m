function optout = vsVHDLset(vs,opts)
% MPCset   Checks and sets the options for the generation of VHDL code for
%          the virtualSensor object
%
% OPTOUT = vsVHDLset(FUN,OPTS)
% FUN is a virtualSensor object, OPTS is the structure defining the options.
% See the documentation of virtualSensor.generateVHDL for a list of all
% structure fields.
...
if ~isfield(opts,'outputRepresentation') || isempty(opts.outputRepresentation)
    optout.outputRepresentation = optout.inputRepresentation;
else
...
if ~isfield(opts,'outputRange') || isempty(opts.outputRange)
    optout.outputRange = optout.inputRange;
else
...
% Input and output representation must be the same if mz > 0
if mz > 0
    if optout.inputRepresentation ~= optout.outputRepresentation
        error('inputRepresentation and outputRepresentation must be the same');
    end
    if optout.inputResolution ~= optout.outputResolution
        error('inputResolution and outputResolution must be the same');
    end
    if optout.inputRange.min(end) ~= optout.outputRange.min(end)
        error('inputRange and outputRange must be the same');
    end
    if optout.inputRange.max(end) ~= optout.outputRange.max(end)
        error('inputRange and outputRange must be the same');
    end
end
...
if mz > 0
    if any(optout.initialCondition < domain.zmin) || any(optout.initialCondition > domain.zmax)
        error('OPTS.initialCondition is outside the virtual sensor domain');
    end
end
