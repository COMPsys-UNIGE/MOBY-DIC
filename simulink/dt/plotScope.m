...
for i = 1:nx
    subplot(nx,1,i)
    hold on
    stairs(res_X.time,res_X.signals.values(:,i),'b')
...
if exist('res_Y','var')
    ...
    for i = 1:ny
        subplot(ny,1,i)
        hold on
        stairs(res_Y.time,res_Y.signals.values(:,i),'b')
...
if exist('res_P','var')
    ...
    for i = 1:np
        subplot(np,1,i)
        hold on
        stairs(res_P.time,res_P.signals.values(:,i),'b')
        xlabel('t')
        ylabel(sim_pnames{i})
        grid on
    end
...
if exist('res_D','var')
    ...
    for i = 1:nd
        subplot(nd,1,i)
        hold on
        stairs(res_D.time,res_D.signals.values(:,i),'b')
